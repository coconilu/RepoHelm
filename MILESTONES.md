...
## 路线北极星：三大核心能力（issue #28）

RepoHelm 的三大核心能力是：以生产级 harness 提供受控工具与运行环境，以动态委派 agent 在能力和成本之间做运行时分配，以测试先行验收把每次改动变成可验证、可审计的交付。

三者的关系：

- harness 配套设施提供真实开发闭环和安全边界。
- 动态委派 agent 在运行时选择合适 worker、backend 和模型层级。
- 测试先行把「现状 / 目标 / 验收标准」固化为 issue、PR、Spec 和交付验证关卡。

| 核心能力 | 默认目标 | 子 issue / PR | 里程碑落点 | 当前追踪 |
| --- | --- | --- | --- | --- |
| Harness 配套设施 | 让 worker 拥有读写、搜索、shell、网络、后台进程、多模态和过程可观测能力，并全部落在权限、sandbox、secrets 和 audit 边界内。 | #22、#23、#25、#27、#6 | M6、M7、M9 | 基础工具、安全加固、命令审批、运行时能力已落地；后续新增工具或 runtime 必须同步权限声明、审计事件和验证用例。 |
| 动态委派 agent | Entry/supervisor agent 根据前序结果动态决定继续委派给哪个 worker，并按任务复杂度匹配能力和成本。 | #4、#7、#26、#6 | M4、M8、M9 | Delegate 模式已接入 tool-calling 循环；后续模型/成本路由、取消和预算约束继续挂在编排与 runtime 里程碑下追踪。 |
| 测试先行 | 每个 feature/fix 先回答现状、目标和验收标准，再写或更新测试，交付时给出可执行验证结果。 | #2、#20、#28、#30 | M1、M5、M9，以及贡献流程 | Feedback/fix 闭环、golden e2e、复用优先和三段式 issue/PR 模板已纳入主干；后续 validation command UI 与 worktree 可运行验证命令继续作为交付关卡打磨。 |

默认 feature/fix 流程：

1. Issue 必须包含「现状 / 目标 / 验收标准」；新能力还必须包含社区方案调研和选型结论。
2. 计划阶段明确映射到三大核心能力之一；如果不属于三者，需要说明为什么仍符合 RepoHelm 产品方向。
3. 实现阶段优先使用既有 harness、agent backend、tools、fixtures 和测试工具；复杂任务默认让 entry agent 在可审计事件流中委派 worker。
4. 交付阶段必须记录最小相关验证命令；影响 Agent 编排、工具、安全、状态或 UI 主流程时，补充单测或 e2e。
...
1. 把 issue #28 的三大核心能力持续映射到子 issue、PR 和里程碑，而不是分散推进。
2. 真正的桌面壳与 standalone binary。
3. 团队 workspace 同步与协作权限模型。
4. 扩展能力市场与第三方 MCP 治理。
5. 编排计划模板化、worker 进程可取消等执行链路打磨。
