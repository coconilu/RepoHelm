...
RepoHelm 的工程原则是复用优先，而不是急于造轮子。遇到新的功能、工具或能力缺口时，第一步应该先调研社区已有方案，再决定复用、封装还是自建。
...
- 现成 CLI、服务和 provider adapter。
- 项目内已经存在的模块、测试 fixture 和工具链。

评估维度必须标准化：

- 契合度：是否匹配 Quest、worktree、审计、知识库和权限边界。
- 维护活跃度：release、commit、issue 响应、社区使用情况。
- 许可证兼容性。
- 安全面：文件系统、网络、命令执行、secrets、供应链风险。
- 集成成本：直接复用、封装 adapter、fork 维护或自建的长期成本。

自建只有在以下情况优先：

- 社区方案契合度低、维护停滞或许可证不兼容。
- 会引入不可接受的安全、权限或供应链风险。
- 能力属于 RepoHelm 的核心护城河：编排、治理、隔离、审计和知识沉淀。
- 封装成本高于清晰的本地实现，并且长期维护边界可控。

选型结论必须可追溯。新 feature issue 应包含社区方案调研和取舍理由；长期架构结论应写入 Repo Wiki 的 `decisions` 页或本文档。

已走通案例：Agent 执行层。RepoHelm 需要 coding-agent 执行能力时，没有自建 agent 内核，而是评估并接入 Codex CLI、Claude Code、OpenCode 等外部 CLI backend。RepoHelm 只在其上实现 worktree 隔离、Spec 关卡、过程审计、状态治理和知识沉淀。

### 3.8 测试先行的交付契约

RepoHelm 的每个非平凡 feature/fix 都应该先形成一个可验证契约，而不是先进入实现。

契约至少包含：

- 现状：当前行为、限制、失败方式或能力缺口。
- 目标：改动完成后用户、Agent 或维护者能获得什么新能力。
- 验收标准：可以被测试、命令、e2e 流程或人工 review 勾选的完成条件。

这个契约贯穿 issue、Spec、计划、worker task、PR 和交付报告。实现时应先补或更新最小相关测试；无法自动化的验收也要记录可复现的人工验证步骤。Agent 执行必须优先使用 RepoHelm harness 暴露的受控工具，并把命令、文件变更、测试输出和 review 结论写入事件流。
...
- 新能力遵循复用优先：先调研社区方案，再决定复用、封装或自建，并记录选型理由。
...
- 它是否先调研并记录了可复用的社区方案？
- 它是否明确落在 harness、动态委派或测试先行三大核心能力之一？
- 它是否把「现状 / 目标 / 验收标准」变成了可验证的交付契约？
