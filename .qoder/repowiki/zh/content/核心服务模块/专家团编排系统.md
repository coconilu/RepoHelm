...
- [docs/superpowers/plans/2026-06-10-expert-orchestration.md](file://docs/superpowers/plans/2026-06-10-expert-orchestration.md)
...
- [packages/core/src/expert/orchestrator.test.ts](file://packages/core/src/expert/orchestrator.test.ts)
</cite>

## 更新摘要
**所做更改**
- 新增详细的专家团编排系统实施计划文档分析
- 更新专家会话管理器、代理池、专家编排器等核心组件的完整实现
- 添加研究收集器、TDD流水线、迁移模块的架构分析
- 完善6个Inspector面板的UI组件实现说明
- 更新实时流式传输(SSE)和状态机管理的新功能
- 完善服务器端和客户端API的集成说明
- 更新状态管理和持久化机制的实现细节
- 新增ExpertOrchestrator编排器的完整实现
- 新增SSE流式传输的完整服务器端和客户端实现
- 新增实时状态更新的完整事件驱动机制
...
7. [UI组件与Inspector面板](#ui组件与inspector面板)
8. [API集成与持久化](#api集成与持久化)
9. [依赖关系分析](#依赖关系分析)
10. [性能考虑](#性能考虑)
11. [故障排除指南](#故障排除指南)
12. [结论](#结论)
...
- **6个Inspector面板**：编排、进展、验收、产物、引用、调研的全景展示
- **实时流式传输**：SSE推送任务状态变化和执行进度
...
### ExpertOrchestrator 编排器实现

ExpertOrchestrator 是专家团编排系统的核心组件，实现了完整的任务分析和分解流程：
...
**章节来源**
- [packages/core/src/expert/orchestrator.ts:1-84](file://packages/core/src/expert/orchestrator.ts#L1-L84)

### SSE 实时流式传输实现

系统实现了完整的 SSE（Server-Sent Events）实时流式传输机制：
...
Client->>Server : GET /api/expert/session/ : id/stream
Server->>Client : SSE 连接建立
loop 执行过程
Server->>Client : 事件 : connected
Server->>Client : 事件 : session_update
Server->>Client : 事件 : task_started
Server->>Client : 事件 : task_progress
Server->>Client : 事件 : task_completed
Server->>Client : 事件 : test_result
Server->>Client : 事件 : research_update
end
Client->>Server : 断开连接
```

**图表来源**
- [apps/server/src/index.ts:894-918](file://apps/server/src/index.ts#L894-L918)

**章节来源**
- [apps/server/src/index.ts:893-918](file://apps/server/src/index.ts#L893-L918)
...
系统提供了完整的6个Inspector面板，用于专家会话的全景展示：

```mermaid
graph TB
subgraph "Inspector 面板"
ORCHESTRATION[编排面板<br/>Agent 示意图 + 任务树]
PROGRESS[进展面板<br/>任务进展列表]
ACCEPTANCE[验收面板<br/>验收用例列表]
DELIVERABLES[产物面板<br/>文件列表 + diff查看]
REFERENCES[引用面板<br/>知识库 + 用户习惯 + 反例]
RESEARCH[调研面板<br/>代码调研结果]
END
subgraph "状态切换"
ANALYZING[analyzing → 自动编排]
AWAITING_CONFIRMATION[awaiting_confirmation → 停编排]
CONFIRMED[confirmed → 自动进展]
EXECUTING[executing → 跟随活跃任务]
COMPLETED[completed → 停产物]
END
...
**图表来源**
- [docs/superpowers/specs/2026-06-10-expert-orchestration-design.md:327-396](file://docs/superpowers/specs/2026-06-10-expert-orchestration-design.md#L327-L396)

### 编排面板 (Orchestration Panel)

编排面板展示了专家团的Agent示意图和任务树结构：
...
OrchestrationPanel --> ExpertAgentDiagram : "渲染"
OrchestrationPanel --> TaskTreeNode : "递归渲染"
TaskTreeNode --> ExpertTaskNode : "使用"
```

**图表来源**
- [apps/web/src/components/OrchestrationPanel.tsx:18-36](file://apps/web/src/components/OrchestrationPanel.tsx#L18-L36)
...
class ExpertProgressRow {
+task ExpertTask
+render() JSX.Element
}
ProgressPanel --> ExpertTask : "遍历"
ProgressPanel --> ExpertProgressRow : "渲染"
```

**图表来源**
- [apps/web/src/components/ProgressPanel.tsx:6-22](file://apps/web/src/components/ProgressPanel.tsx#L6-L22)

### 验收面板 (Acceptance Panel)
...
class ExpertAcceptanceCard {
+test AcceptanceTest
+render() JSX.Element
}
AcceptancePanel --> AcceptanceTest : "管理"
AcceptancePanel --> ExpertAcceptanceCard : "渲染"
```

**图表来源**
- [apps/web/src/components/AcceptancePanel.tsx:6-26](file://apps/web/src/components/AcceptancePanel.tsx#L6-L26)

### 产物面板 (Deliverables Panel)
...
class ExpertDiffViewer {
+artifact TaskArtifact
+render() JSX.Element
}
DeliverablesPanel --> TaskArtifact : "筛选"
DeliverablesPanel --> ExpertDiffViewer : "渲染"
```

**图表来源**
- [apps/web/src/components/DeliverablesPanel.tsx:17-33](file://apps/web/src/components/DeliverablesPanel.tsx#L17-L33)
...
CHAT[POST /api/expert/session/:id/chat<br/>对话入口 Agent]
TASK[PATCH /api/expert/session/:id/task/:taskId<br/>修改任务]
END
subgraph "存储层"
...
**图表来源**
- [docs/superpowers/specs/2026-06-10-expert-orchestration-design.md:423-438](file://docs/superpowers/specs/2026-06-10-expert-orchestration-design.md#L423-L438)
...
1. **检查连接状态**：验证 SSE 连接是否正常建立
2. **监控事件推送**：确认事件类型和数据格式正确
3. **调试网络问题**：检查防火墙和代理设置
4. **重启流连接**：断开后自动重连机制
...
9. **6个Inspector面板**：全景展示专家会话的各个方面
10. **实时流式传输**：SSE推送执行状态和进度更新
...
| REPOHELM_FAKE_MODELS | 模拟模型开关 | 未设置 |
...
// 实时监听会话状态
const eventSource = new EventSource(`/api/expert/session/${session.id}/stream`);
