...
## 更新摘要
**变更内容**
- 新增多代理编排功能，包括 SubAgentOrchestrator 和子代理生命周期管理
- 增强状态管理机制，引入原子状态突变处理
- 添加内置子代理种子功能，支持 Supervisor-Worker 模式的多代理协作
- 扩展 ModelKit 管理功能，支持 CLI 和 BYOK 两种模式
...
- **新增**：多代理编排架构与子代理生命周期管理
- **新增**：原子状态突变处理机制
- **新增**：内置子代理种子功能与 Supervisor-Worker 模式
- 提供使用模式与代码示例路径
- 说明与其他组件的集成关系
- 给出常见问题与性能优化建议
...
- **新增**：SubAgentOrchestrator：子代理编排器，实现 Supervisor-Worker 模式的多代理协作，支持任务分解、委派和结果汇总
...
- **新增**：多代理编排：通过 SubAgentOrchestrator 实现 Supervisor-Worker 模式的任务委派和协作
...
### 多代理编排架构
- **新增**：SubAgentOrchestrator
  - 计划生成：基于入口子代理和代理池生成结构化执行计划
  - 任务委派：按照依赖关系顺序委派给子代理执行
  - 结果汇总：收集子代理执行结果，生成最终摘要
  - 工具调用：支持文件系统工具调用，实现 BYOK 模型的文件写入能力
- **新增**：内置子代理种子
...
- **新增**：编排阶段
  - runQuest：解析入口子代理，生成编排计划，创建计划文件，移动到执行状态
  - approvePlan：批准编排计划，创建工作树，执行已批准计划，收集变更文件
- 执行阶段
  - provisionQuestWorktrees：为受影响项目创建工作树，确保隔离执行环境
  - executeApprovedPlan：通过 SubAgentOrchestrator 执行已批准计划，支持任务委派和依赖管理
- 交付阶段
