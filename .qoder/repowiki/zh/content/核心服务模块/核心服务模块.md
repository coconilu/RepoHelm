...
## 更新摘要
**变更内容**
- 新增专家编排相关方法：createExpertSession、getExpertSession、updateExpertSession 的完整实现
- 扩展 StateStore 以支持专家会话的持久化存储
- 完善 ExpertSession 类型定义和相关数据结构
- 新增 ExpertSessionManager 状态管理机制
- 扩展 AgentPool 管理动态专家代理的能力
- 新增 ResearchCollector 代码调研收集功能
- 新增 TDD Pipeline 测试驱动执行流程
- 新增 ExpertOrchestrator 主编排器的完整实现
...
RepoHelm 以"虚拟 workspace + 多项目 Quest + Spec 驱动 + worktree 隔离 + Agent 编排 + 知识库"为核心产品方向，核心服务模块负责状态持久化、工作流编排、安全策略与审计、以及与 Git、Provider、CLI、Agent 的集成。**新增**的专家团编排系统将传统的静态 Plan-then-Execute 模式升级为动态专家团模式，提供更智能、更灵活的任务执行和管理能力。
...
participant EO as "ExpertOrchestrator"
participant SM as "ExpertSessionManager"
participant AP as "Agent Pool"
participant RC as "代码调研收集器"
participant TP as "TDD Pipeline"
participant O as "SubAgentOrchestrator"
U->>S : 创建专家会话
S->>SM : 创建 ExpertSession
SM->>AP : 加载专家原型
SM->>RC : 伴随代码调研
RC-->>SM : 返回调研结果
SM->>SM : 生成任务树和验收用例
...
这些新增功能将 RepoHelm 从传统的静态编排模式升级为动态专家团模式，显著提升了系统的智能化水平、灵活性和执行效率，为复杂项目的自动化管理和智能执行提供了强有力的支持。
