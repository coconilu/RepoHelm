...
## 更新摘要
**所做更改**
- 新增 ModelKit 显示组件和管理界面章节
- 添加 ModelKit 卡片布局和类型指示器说明
- 新增子代理管理功能介绍
- 更新应用设置界面以包含 ModelKit 和子代理标签页
- 新增 ModelKit 和子代理相关的样式定义说明
- 新增计划审批面板和实时状态指示器功能说明
- 新增子代理创建/编辑对话框组件
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [ModelKit 显示组件与管理界面](#modelkit-显示组件与管理界面)
7. [子代理管理界面](#子代理管理界面)
8. [计划审批面板与实时状态指示器](#计划审批面板与实时状态指示器)
9. [依赖关系分析](#依赖关系分析)
10. [性能考虑](#性能考虑)
11. [故障排查指南](#故障排查指南)
12. [结论](#结论)
...
**更新** 本版本新增了 ModelKit 显示组件、配置界面和管理流程的详细说明，包括 ModelKit 列表、卡片布局、类型指示器和操作按钮。同时新增了子代理管理功能，支持创建、编辑、删除和设置入口代理。还新增了计划审批面板和实时状态指示器，提供更完善的任务管理体验。
...
### ModelKit 管理状态与功能
- 状态管理
  - modelKits：存储所有已保存的 ModelKit 列表
  - savingAsKit：保存为 ModelKit 的临时状态，包含类型、后端ID、提供商ID、模型等信息
  - kitDraft：ModelKit 草稿状态，主要用于保存时的名称输入
  - editingKit：当前编辑的 ModelKit 对象
  - kitError：ModelKit 操作错误信息
- 初始化加载
  - 在组件挂载时并发加载 CLI 列表、引擎配置、ModelKit 列表、子代理列表和入口子代理
  - 设置当前活动的 BYOK 提供商配置

### ModelKit 列表与卡片布局
- 列表展示
  - 在应用设置的 "modelkits" 标签页中展示所有 ModelKit
  - 使用 modelkit-list 容器和 modelkit-card 卡片布局
  - 每个 ModelKit 卡片包含头部信息、详情和操作按钮
- 卡片组件结构
  - 头部：包含 ModelKit 名称和类型指示器（CLI/BYOK）
  - 操作：编辑和删除按钮
  - 详情：显示模型标识、后端ID、提供商ID和创建时间
- 类型指示器
  - 使用 modelkit-type 类显示圆形徽章
  - CLI 类型显示为 "CLI"，BYOK 类型显示为 "BYOK"

### ModelKit 操作功能
- 保存为 ModelKit
  - 通过 saveAsModelKit 函数实现，支持 CLI 和 BYOK 两种类型
  - 自动测试模型配置的有效性后再保存
  - 默认成本等级为 "medium"，性能配置为 "balanced"
- 更新 ModelKit
  - 通过 updateModelKitHandler 函数更新现有配置
  - 支持修改名称、模型和配置参数
- 删除 ModelKit
  - 通过 deleteModelKitHandler 函数删除指定 ID 的 ModelKit
  - 包含确认对话框防止误删
...
```mermaid
flowchart TD
ModelKit["ModelKit 管理"] --> List["ModelKit 列表"]
ModelKit --> Card["卡片布局"]
ModelKit --> Actions["操作按钮"]
Card --> Header["头部信息"]
Card --> Details["详情信息"]
Card --> TypeBadge["类型指示器"]
Actions --> Edit["编辑"]
Actions --> Delete["删除"]
Actions --> Save["保存为 ModelKit"]
```

**图表来源**
- [apps/web/src/App.tsx:1506-1520](file://apps/web/src/App.tsx#L1506-L1520)
- [apps/web/src/App.tsx:2117-2162](file://apps/web/src/App.tsx#L2117-L2162)
- [apps/web/src/styles.css:2306-2380](file://apps/web/src/styles.css#L2306-L2380)

**章节来源**
- [apps/web/src/App.tsx:1506-1732](file://apps/web/src/App.tsx#L1506-L1732)
- [apps/web/src/App.tsx:2117-2262](file://apps/web/src/App.tsx#L2117-L2262)
- [apps/web/src/styles.css:2306-2499](file://apps/web/src/styles.css#L2306-L2499)
...
- 状态管理
  - subAgents：存储所有子代理列表
  - creatingSubAgent：创建子代理的临时状态
  - editingSubAgent：编辑中的子代理对象
  - subAgentError：子代理操作错误信息
- 初始化加载
  - 在组件挂载时并发加载子代理列表和 ModelKit 列表
  - 设置当前活动的入口子代理

### 子代理列表与卡片布局
- 列表展示
  - 在应用设置的 "subagents" 标签页中展示所有子代理
  - 支持创建、编辑和删除子代理
  - 显示子代理的模式（入口/工作节点）、角色、绑定的 ModelKit 和权限配置
- 卡片组件结构
  - 头部：包含子代理名称、入口标识和模式标签
  - 操作：设为入口、编辑和删除按钮
  - 详情：显示角色、绑定的 ModelKit、能力标签和权限配置
- 权限显示
  - 允许工具和禁止工具的可视化展示
  - 最大执行步数的配置显示
  - 创建时间的元数据展示
...
### 子代理操作功能
- 创建子代理
  - 通过 createSubAgentHandler 函数实现，支持完整的配置流程
  - 包括基本信息、ModelKit 绑定、权限配置、模式选择和系统提示模板
- 更新子代理
  - 通过 updateSubAgentHandler 函数更新现有配置
  - 支持修改名称、角色、能力标签、ModelKit 绑定和权限配置
- 删除子代理
  - 通过 deleteSubAgentHandler 函数删除指定 ID 的子代理
  - 包含入口代理保护和确认对话框
- 设置入口子代理
  - 通过 setEntrySubAgentHandler 函数设置入口代理
  - 更新入口代理标识和全局状态

### 子代理对话框组件
- SubAgentDialog
  - 支持创建和编辑两种模式
  - 分步骤配置：基本信息、ModelKit 绑定、权限配置、模式选择、系统提示模板
  - 能力标签和工具权限的可视化配置
  - 表单验证和错误处理

```mermaid
flowchart TD
SubAgent["子代理管理"] --> List["子代理列表"]
SubAgent --> Dialog["创建/编辑对话框"]
SubAgent --> Actions["操作按钮"]
List --> Card["子代理卡片"]
Card --> Header["头部信息"]
Card --> Details["详情信息"]
Card --> Actions["操作按钮"]
Dialog --> Steps["配置步骤"]
Steps --> Basic["基本信息"]
Steps --> Kit["ModelKit 绑定"]
Steps --> Permissions["权限配置"]
Steps --> Mode["模式选择"]
Steps --> Prompt["系统提示模板"]
```

**图表来源**
- [apps/web/src/App.tsx:2100-2160](file://apps/web/src/App.tsx#L2100-L2160)
- [apps/web/src/App.tsx:2522-2620](file://apps/web/src/App.tsx#L2522-L2620)
- [apps/web/src/App.tsx:3231-3543](file://apps/web/src/App.tsx#L3231-L3543)
...
### 计划审批面板
- 审批功能
  - approvePlan：批准当前 Quest 的编排计划
  - rejectPlan：拒绝当前 Quest 的编排计划，支持提供拒绝原因
  - 审批状态跟踪：显示计划审批的状态和时间戳
- 用户交互
  - 审批按钮：批准计划并开始执行步骤
  - 拒绝按钮：拒绝计划并提供拒绝原因
  - 状态指示：显示当前计划审批状态和决策结果
...
- 状态映射
  ...
  - 取消 → cancelled
- 状态样式
  - 不同状态对应不同的徽章样式（蓝色、绿色、红色等）
  - 状态标签的本地化显示（中文状态标签）

### 状态管理与更新
- 状态更新流程
  - 审批操作完成后重新加载状态
  - 更新 Inspector 标签页到相应的状态视图
  - 保持用户界面的实时同步
- 错误处理
  - 审批操作失败时显示错误信息
  - 恢复用户输入状态，避免数据丢失
...
```mermaid
flowchart TD
Plan["计划审批"] --> Approve["批准计划"]
Plan --> Reject["拒绝计划"]
Approve --> Update["更新状态"]
Reject --> Update
Update --> Reload["重新加载数据"]
Reload --> Inspector["更新 Inspector 视图"]
Inspector --> Status["显示新状态"]
```

**图表来源**
- [apps/web/src/App.tsx:313-345](file://apps/web/src/App.tsx#L313-L345)
- [apps/web/src/App.tsx:636-642](file://apps/web/src/App.tsx#L636-L642)
...
- 常见问题
  ...
  - ModelKit 无法保存：检查模型配置的有效性，确认 API 端点正常工作。
  - 子代理操作失败：检查 ModelKit 绑定是否有效，确认权限配置正确。
- 错误提示
...
**更新** 新增的 ModelKit 显示组件和管理界面、子代理管理功能以及计划审批面板进一步增强了应用的配置管理能力和任务管理体验，提供了直观的卡片式界面来管理和组织模型配置与智能体，同时保持了与现有架构的一致性和可扩展性。
...
- 可访问性
  ...
  - ModelKit 和子代理卡片提供清晰的视觉层次和语义化结构。
...
- 与组件的耦合
  ...
  - ModelKit 和子代理样式完全基于 CSS 变量，支持无缝主题切换。
...
- 与根组件的集成
  ...
  - ModelKit 管理功能作为应用设置的一部分集成。
  - 子代理管理功能作为应用设置的一部分集成。
- 与 API 的集成
  - 通过 api.ts 的函数式接口调用后端，统一错误处理与返回类型。
  - ModelKit 和子代理操作通过专门的 API 函数实现。
- 与样式系统的集成
  - 使用 cn 工具合并类名，确保组件在不同主题下保持一致外观。
  - ModelKit 和子代理样式完全基于 CSS 变量系统。
...
### ModelKit 管理界面集成
- 应用设置标签页
  - 新增 "modelkits" 标签页用于 ModelKit 管理
  - 新增 "subagents" 标签页用于子代理管理
- 交互流程
  - 从模型管理界面保存为 ModelKit
  - 在设置界面查看、编辑和删除 ModelKit
  - 管理子代理的创建、配置和权限设置

**章节来源**
- [apps/web/src/App.tsx:83](file://apps/web/src/App.tsx#L83)
- [apps/web/src/App.tsx:2117-2262](file://apps/web/src/App.tsx#L2117-L2262)