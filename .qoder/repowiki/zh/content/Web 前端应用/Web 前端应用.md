...
## 更新摘要
**所做更改**
- 更新Inspector组件章节，反映从静态标签系统到动态标签管理的重大UI变更
- 移除旧的安全面板、产品面板和时间线组件的相关内容
- 新增分支自动检测功能的技术实现和使用说明
- 更新Inspector组件的动态标签系统和自动选择机制
- 新增分支检测API集成和自动默认分支设置功能
- 更新架构总览，反映Inspector组件的现代化设计
...
**更新** 本版本重点反映了Inspector组件的重大UI变更：从传统的静态标签系统迁移到动态标签管理系统，实现了根据内容可用性自动显示和隐藏标签页的功能。同时新增了分支自动检测功能，能够智能识别Git仓库的默认分支并自动设置项目配置。
...
- [apps/web/src/App.tsx:1-3488](file://apps/web/src/App.tsx#L1-L3488)
...
**更新** 新增的Inspector动态标签管理系统和分支自动检测功能进一步增强了应用的智能化水平。动态标签系统实现了真正的按需显示，避免了空标签页的出现，提升了用户体验。分支自动检测功能简化了项目配置流程，减少了用户的操作负担。知识中心组件、嵌入模型配置选项和改进的 Quest 组合功能进一步增强了应用的知识管理和任务协作能力。最新的项目过滤逻辑变更使得知识中心能够跨工作区访问所有注册项目的知识库，显著提升了知识管理的灵活性和用户效率。
...
### 组件组合模式与集成
- 与根组件的集成
  - App 作为容器，将 CommandPalette、Select、KnowledgeCenter、Inspector 等组件作为局部功能模块嵌入。
