...
## 更新摘要
**所做更改**
- 新增知识中心UI重大增强，包括KnowledgeCenter组件、MarkdownView、MermaidDiagram组件的完整实现
...
**更新** 新增了完整的知识中心UI组件系统，包括KnowledgeCenter主面板、MarkdownView渲染组件、MermaidDiagram图表渲染组件，以及增强的API错误处理机制，为知识库相关端点提供精确的HTTP状态码响应（404/400）。UI层采用现代化的设计语言，支持深色/浅色主题切换，提供流畅的用户体验。
...
- **知识中心面板（KnowledgeCenter）**
  - **新增** 完整的知识中心主面板组件，占据应用右侧区域，采用网格布局设计
...
## 架构总览
新知识库系统采用三层架构：Repo Wiki管理层负责知识库的创建和维护，Wiki存储层提供持久化存储，向量处理层实现智能检索。**更新** 增强的错误处理机制确保API调用的可靠性和一致性。**新增** 完整的UI组件系统提供现代化的知识中心面板体验，支持Repo Wiki和记忆（memory）双模式切换。
