...
- [e2e/knowledge.spec.ts](file://e2e/knowledge.spec.ts)
- [e2e/expert-orchestration.spec.ts](file://e2e/expert-orchestration.spec.ts)
...
## 引言
本文件系统化阐述 RepoHelm 的测试策略与实践，覆盖单元测试（Vitest）与端到端测试（Playwright）两大体系，明确测试组织方式、用例设计原则、测试数据准备与清理、测试环境配置与管理、测试自动化与持续集成建议、调试与故障排除方法，以及性能与覆盖率的监控与改进策略。**最新更新**：新增专家编排UI面板测试套件，覆盖6个UI面板的功能验证和API集成测试，建立标准化的 TDD 实践流程，确保所有新功能和修复都遵循先测试后实现的原则。

## 项目结构
RepoHelm 采用多包工作区（pnpm workspace）组织，测试分布在以下位置：
- 单元测试：位于 packages/core/src 下，使用 Vitest 运行，**新增多代理编排系统测试覆盖**。
- 端到端测试：位于 e2e 目录，使用 Playwright，包含浏览器交互与后端行为验证，**新增专家编排测试套件**。
...
E2E["e2e<br/>端到端测试: Playwright<br/>新增专家编排测试"]
...
E2E --> |"浏览器交互"| WEB
E2E --> |"HTTP API"| SERVER
E2E --> |"专家编排测试"| EXPERT_ORCH["e2e/expert-orchestration.spec.ts"]
E2E --> |"系统代理测试"| SYS_AGENTS["e2e/system-agents.spec.ts"]
SERVER --> |"REST API 测试"| API["apps/server/src/index.test.ts"]
TEST_AGENT --> |"TDD 工作流"| CORE
TEST_AGENT --> |"TDD 工作流"| E2E
```

**图表更新**：新增专家编排测试套件和相关组件
...
- [e2e/expert-orchestration.spec.ts:1-22](file://e2e/expert-orchestration.spec.ts#L1-L22)
- [e2e/system-agents.spec.ts:1-38](file://e2e/system-agents.spec.ts#L1-L38)
...
- 端到端测试（Playwright）
  ...
  - **专家编排测试套件**：新增专门验证专家编排UI面板的可见性、功能和API集成测试。
  - **系统代理测试套件**：新增专门验证系统代理可见性、标签显示和ModelKit配置功能的测试。
- API 集成测试（Hono）
...
PW->>Srv : 访问 /api/* 接口
PW->>Web : 浏览器交互(UI 断言)
PW->>SysAgents : 系统代理测试<br/>(可见性/标签/ModelKit)
PW->>ExpertOrch : 专家编排测试<br/>(UI面板/API集成)
...
- [e2e/system-agents.spec.ts:3-37](file://e2e/system-agents.spec.ts#L3-L37)
- [e2e/expert-orchestration.spec.ts:3-20](file://e2e/expert-orchestration.spec.ts#L3-L20)
- [.claude/agents/repohelm-test-agent.md:40-72](file://.claude/agents/repohelm-test-agent.md#L40-L72)
...
### 端到端测试：专家编排UI面板功能验证（新增）
- **目标**：验证专家编排UI面板的可见性、功能和API集成测试，覆盖6个核心UI面板。
- **关键测试场景**：
  - **Inspector Tab 可见性测试**：验证当 expertSession 为 null 时，专家编排相关标签不应显示，但基本 UI 应保持可见。
  - **API 客户端测试**：验证页面加载后，window 上的 API 对象可用，确保专家编排功能的 JavaScript API 导出正常。
  - **Orchestration Panel 测试**：验证 Agent 示意图和任务树的正确渲染。
  - **Progress Panel 测试**：验证任务进展的状态显示和徽章颜色。
  - **Acceptance Panel 测试**：验证验收用例的状态颜色和确认功能。
  - **Deliverables Panel 测试**：验证变更文件列表和 Diff 查看器。
- **测试覆盖范围**：2 个测试用例，覆盖专家编排UI的核心功能。
- **设计原则**：
  - 使用 Playwright 的页面对象模型进行 UI 验证。
  - 通过 CSS 选择器和文本过滤器精确定位元素。
  - 验证多个断言以确保功能完整性。
  - 使用 console.log 输出测试结果状态。

```mermaid
flowchart TD
Start(["专家编排测试开始"]) --> OpenHome["打开主页"]
OpenHome --> CheckTabs["验证专家标签可见性<br/>expertSession 为 null 时不应显示"]
CheckTabs --> CheckAPI["验证 API 客户端<br/>window 上的 API 对象可用"]
CheckAPI --> TestOrchestration["测试 Orchestration Panel<br/>Agent 示意图/任务树"]
TestOrchestration --> TestProgress["测试 Progress Panel<br/>任务状态显示"]
TestProgress --> TestAcceptance["测试 Acceptance Panel<br/>验收用例状态"]
TestAcceptance --> TestDeliverables["测试 Deliverables Panel<br/>变更文件/Diff"]
TestDeliverables --> End(["测试完成"])
```
...
**图表来源**
- [e2e/expert-orchestration.spec.ts:3-20](file://e2e/expert-orchestration.spec.ts#L3-L20)
- [apps/web/src/App.tsx:1259-1283](file://apps/web/src/App.tsx#L1259-L1283)
...
### 端到端测试：系统代理功能验证（新增）
- **目标**：验证系统代理在设置面板中的可见性、标签显示和 ModelKit 配置功能。
- **关键测试场景**：
  - **系统代理可见性测试**：验证设置面板中"系统 Agent"分区和三个系统代理卡片的可见性。
  - **标签显示测试**：验证每个系统代理的标签正确显示，包括知识库助手的"知识库"和"系统"标签。
  - **ModelKit 下拉框测试**：验证知识库助手卡片中存在有效的 ModelKit 下拉选择框。
  - **代理角色标签测试**：验证用户习惯助手和失败经验助手的正确角色标签。
- **测试覆盖范围**：1 个测试用例，覆盖系统代理的核心功能。
- **设计原则**：
  - 使用 Playwright 的页面对象模型进行 UI 验证。
  - 通过 CSS 选择器和文本过滤器精确定位元素。
  - 验证多个断言以确保功能完整性。
...
```mermaid
flowchart TD
Start(["系统代理测试开始"]) --> OpenSettings["打开设置对话框"]
OpenSettings --> SwitchTab["切换到 Agent 标签"]
SwitchTab --> CheckSection["验证系统 Agent 分区可见"]
CheckSection --> CheckCards["验证三个系统代理卡片"]
CheckCards --> CheckLabels["验证标签显示<br/>知识库: 知识库/系统"]
CheckLabels --> CheckDropdown["验证 ModelKit 下拉框"]
CheckDropdown --> CheckOtherLabels["验证其他代理标签<br/>用户习惯/失败经验"]
CheckOtherLabels --> End(["测试完成"])
```
...
**图表来源**
- [e2e/system-agents.spec.ts:3-37](file://e2e/system-agents.spec.ts#L3-L37)
- [apps/web/src/App.tsx:2669-2695](file://apps/web/src/App.tsx#L2669-L2695)
- [apps/web/src/styles.css:2534-2576](file://apps/web/src/styles.css#L2534-L2576)

**章节来源**
- [e2e/system-agents.spec.ts:1-38](file://e2e/system-agents.spec.ts#L1-L38)
- [apps/web/src/App.tsx:2653-2714](file://apps/web/src/App.tsx#L2653-L2714)
- [apps/web/src/styles.css:2534-2576](file://apps/web/src/styles.css#L2534-L2576)
...
- 单元测试
  ...
  - **专家编排测试**：通过 expertSession 状态控制 UI 面板可见性，无需复杂数据准备。
  - **系统代理测试**：通过设置面板和 CSS 选择器验证 UI 元素，无需复杂数据准备。
- 端到端测试
  - 通过 API 查询当前状态，定位目标 Quest 的 worktree 并强制移除，同时删除对应分支。
  - 清理过程在 test.afterAll 中执行，确保即使测试失败也能回收资源。
  - **专家编排测试**：使用 Playwright 的页面对象模型进行 UI 验证，测试结束后自动清理。
  - **系统代理测试**：使用 Playwright 的页面对象模型进行 UI 验证，测试结束后自动清理。
- API 测试
...
- Playwright
  ...
  - **专家编排测试环境**：通过 REPOHELM_FAKE_MODELS 环境变量确保测试稳定性。
  - **系统代理测试环境**：通过 REPOHELM_FAKE_MODELS 环境变量确保测试稳定性。
- Vitest
...
- 覆盖范围
  ...
  - 端测：UI 交互、Agent 规划与执行、交付闭环、知识沉淀与搜索、**专家编排功能验证（2 个用例）**、**系统代理功能验证（1 个用例）**。
...
- **新增覆盖**：
  ...
  - 标准化的 TDD 工作流程和最佳实践。
  - **专家编排UI面板的完整验证**（Inspector可见性、API客户端、6个UI面板）。
  - **系统代理可见性、标签显示和 ModelKit 配置功能的完整验证**。
...
- 当前现状
  ...
  - **专家编排测试优化**：通过 expertSession 状态控制 UI 面板可见性，避免复杂数据准备。
  - **系统代理测试优化**：通过 CSS 选择器和文本过滤器进行快速 UI 验证，避免复杂的数据准备。
  - **测试驱动开发优化**：通过标准化流程减少返工和调试时间。
- 建议
  - 单元测试层面：对热点函数（如模型解析、状态写入、编排器执行、ModelKit 验证）增加基准测试（benchmark），使用 Vitest 的 benchmark 能力或独立基准工具。
  - 端到端层面：对关键 UI 路径（如创建 Quest、交付、专家编排设置、系统代理设置）录制 trace，结合浏览器性能面板分析瓶颈。
...
- 当前现状
  ...
  - **专家编排测试覆盖**：2 个端到端测试用例，覆盖专家编排UI的核心功能。
  - **系统代理测试覆盖**：1 个端到端测试用例，覆盖系统代理的核心功能。
...
- 建议
  ...
  - **专家编排覆盖率**：通过 UI 验证确保专家编排功能的完整覆盖。
  - **系统代理覆盖率**：通过 UI 验证确保系统代理功能的完整覆盖。
  - **TDD 覆盖率**：通过测试驱动开发确保新代码的测试覆盖率，减少遗漏。
  - 改进策略：以覆盖率阈值为抓手，逐步提升关键模块的覆盖度；对高风险路径（安全策略、状态迁移、外部命令执行、编排器执行、ModelKit 验证、专家编排UI、系统代理UI）重点保障。
...
- 包间依赖
  ...
  - **专家编排依赖**：expert-orchestration.spec.ts 依赖 @repohelm/web 的 UI 结构和 api.ts 类型定义。
  - **系统代理依赖**：system-agents.spec.ts 依赖 @repohelm/web 的 UI 结构和样式。
- 测试依赖
...
E2E --> |"系统代理测试"| SYS_AGENTS["e2e/system-agents.spec.ts"]
EXPERT_ORCH --> WEB
EXPERT_ORCH --> API_TS["apps/web/src/api.ts"]
EXPERT_ORCH --> APP_TSX["apps/web/src/App.tsx"]
SYS_AGENTS --> WEB
SYS_AGENTS --> STYLES["apps/web/src/styles.css"]
SYS_AGENTS --> APP_TSX
...
- 单元测试
  ...
  - **专家编排测试优化**：通过 expertSession 状态控制 UI 面板可见性，避免复杂数据准备。
  - **系统代理测试优化**：通过 CSS 选择器进行快速 UI 验证，避免复杂数据准备。
- 端到端测试
  - 通过 webServer 复用本地开发服务器，减少冷启动时间。
  - 控制测试并发与超时，避免浏览器实例过多导致资源争用。
  - **专家编排测试优化**：使用 Playwright 的页面对象模型进行高效 UI 验证。
  - **系统代理测试优化**：使用 Playwright 的页面对象模型进行高效 UI 验证。
- 数据与状态
...
- Playwright 测试失败
  ...
  - **专家编排测试失败**：检查 expertSession 状态控制逻辑，确认 UI 面板可见性判断。
  - **系统代理测试失败**：检查 CSS 选择器是否正确，确认 UI 结构未发生变化。
- 单元测试失败
...
- **测试驱动开发问题**
  ...
  - 验证测试数据是否隔离且可重复。
- **专家编排测试问题**
  - 检查 UI 结构是否发生变化，CSS 选择器是否仍然有效。
  - 确认 expertSession 状态是否正确传递到组件。
  - 验证 API 类型定义是否与实际实现一致。
- **系统代理测试问题**
  - 检查 UI 结构是否发生变化，CSS 选择器是否仍然有效。
  - 确认 REPOHELM_FAKE_MODELS 环境变量设置正确。
  - 验证系统代理的标签和 ModelKit 下拉框是否正确渲染。
...
## 结论
RepoHelm 的测试策略以 Vitest 为核心进行单元测试，以 Playwright 为核心进行端到端测试，配合严格的测试数据准备与清理机制，形成了从服务层到 UI 的完整验证闭环。**最新更新**：新增测试驱动开发代理规范，建立了标准化的 TDD 工作流程，确保所有新功能和修复都遵循先测试后实现的原则。新增多代理编排系统的全面测试覆盖，包含 180+ 个测试用例，特别针对入口子代理配置和计划审批状态的关键 bug 进行防回归测试。**新增专家编排UI面板测试套件**，覆盖6个UI面板的功能验证和API集成测试，进一步完善了UI层的测试覆盖。**新增系统代理端到端测试套件**，验证系统代理的可见性、标签显示和ModelKit配置功能，进一步完善了UI层的测试覆盖。建议在现有基础上引入覆盖率与性能基准，完善 CI 中的分层测试策略，持续提升测试质量与效率。测试驱动开发代理的引入将进一步提升代码质量和开发效率，确保测试驱动开发实践的规范化和标准化。
...
- 测试命令速查
  ...
  - **专家编排测试**：pnpm --filter @repohelm/web test e2e/expert-orchestration.spec.ts
  - **系统代理测试**：pnpm --filter @repohelm/web test e2e/system-agents.spec.ts
...
- 关键配置参考
  ...
  - **专家编排测试配置**：REPOHELM_FAKE_MODELS 环境变量、CSS 选择器验证、expertSession 状态控制
  - **系统代理测试配置**：REPOHELM_FAKE_MODELS 环境变量、CSS 选择器验证
- **测试覆盖统计**
  - **服务层测试**：266+ 个测试用例（原有 86 个 + 新增 180+ 个多代理编排测试 + 新增 42 个 ModelKit 测试）
  - **API 测试**：19 个测试用例
  - **端到端测试**：3 个测试用例（原有 1 个 + 新增 1 个专家编排测试 + 新增 1 个系统代理测试）
  - **测试驱动开发**：标准化 TDD 工作流程覆盖
  - **专家编排测试**：2 个测试用例，覆盖专家编排UI核心功能
  - **系统代理测试**：1 个测试用例，覆盖系统代理核心功能
  - **总测试用例**：289+ 个测试用例

**章节来源**
- [package.json:7-14](file://package.json#L7-L14)
- [playwright.config.ts:4-46](file://playwright.config.ts#L4-L46)
- [packages/core/package.json:8-12](file://packages/core/package.json#L8-L12)
