...
## 更新摘要
**所做更改**
- 新增专家编排UI面板相关的API端点：SSE流式传输端点 `/api/expert/session/:id/stream`，以及各个面板对应的API端点
- 新增专家编排UI面板组件：进度面板、验收面板、交付物面板、参考资料面板、研究面板
- 新增专家编排API端点：`/api/expert/session/:id/progress`、`/api/expert/session/:id/acceptance`、`/api/expert/session/:id/deliverables`、`/api/expert/session/:id/references`、`/api/expert/session/:id/research`
- 新增SSE流式传输支持，用于实时推送专家会话状态更新
...
8. [ModelKit 配置模式和验证结构](#modelkit-配置模式和验证结构)
9. [系统代理与智能助手](#系统代理与智能助手)
10. [用户偏好与学习系统](#用户偏好与学习系统)
11. [失败模式与经验管理](#失败模式与经验管理)
12. [依赖分析](#依赖分析)
13. [性能考量](#性能考量)
14. [故障排查指南](#故障排查指南)
15. [结论](#结论)
...
## 简介
本文件系统性梳理 RepoHelm 的数据模型与类型系统，聚焦以下实体与关系：Workspace、Project、Quest、AgentEvent、WorktreeState、ChangedFile、DeliveryState、KnowledgeItem、CapabilityDefinition、SecurityPolicy、AuditLogEntry、EngineConfig、ModelCacheEntry、ModelKit、SubAgent、UserPreference、FailurePattern、ExpertSession、ExpertTaskNode、ExpertTask、AcceptanceTest、CodeResearchResult、AgentPool、TaskArtifact、ExpertError 等。文档涵盖字段定义、数据验证与业务规则、状态存储与缓存策略、数据生命周期与迁移路径、安全与访问控制、扩展与定制指南，以及类型系统的最佳实践与性能优化建议。

## 项目结构
RepoHelm 的数据模型主要位于 packages/core/src/types.ts，状态持久化与迁移在 store.ts，业务编排与工作流在 service.ts，Agent 后端与 Git 操作在 agent.ts 与 git.ts，知识库文件落盘在 knowledge.ts，模型提供方注册与缓存在 providers.ts 与 cli.ts，ModelKit 配置管理在服务端 API 中实现。新增的系统代理、用户偏好、失败模式和专家编排功能通过专门的工具模块和 API 端点实现。
...
subgraph "业务编排"
B1["service.ts<br/>RepoHelmService<br/>工作流/校验/交付<br/>新增系统代理/偏好/失败模式/专家编排管理"]
E3["expert orchestrator<br/>ExpertOrchestrator<br/>任务分解与编排"]
E4["session manager<br/>ExpertSessionManager<br/>会话生命周期管理"]
end
subgraph "专家编排组件"
EP1["expert/orchestrator.ts<br/>专家编排器"]
EP2["expert/session-manager.ts<br/>会话管理器"]
EP3["expert/research-collector.ts<br/>研究收集器"]
EP4["expert/agent-pool.ts<br/>代理池"]
EP5["expert/agent-prototypes.ts<br/>内置专家原型"]
EP6["expert/tdd-pipeline.ts<br/>TDD 流水线"]
end
subgraph "系统代理"
...
subgraph "服务端 API"
API["apps/server/src/index.ts<br/>系统代理/偏好/失败模式/专家编排 API<br/>Zod 验证"]
...
- [service.ts:1-2690](file://packages/core/src/service.ts#L1-L2690)
...
- [index.ts:113-904](file://apps/server/src/index.ts#L113-L904)
...
## 架构总览
RepoHelm 的数据流围绕 RepoHelmService 展开：读取/写入状态（store.ts），创建/更新 Workspace/Project/Quest，驱动 Agent 后端执行，操作 Git worktree，生成事件与知识条目，维护安全策略与审计日志，缓存模型列表，管理 ModelKit 和 SubAgent 配置。新增的系统代理功能通过独立的工具集和 API 端点实现，支持知识库管理、用户偏好学习和失败模式经验积累。专家编排系统通过 ExpertOrchestrator 和 ExpertSessionManager 提供高级任务分解和执行能力。
...
Client->>API : 调用专家编排/系统代理/偏好/失败模式 API
API->>Service : 验证输入参数
...
CheckLegacy --> |存在| Migrate["迁移 byok 至 byokProviders<br/>添加 modelKits/userPreferences/failurePatterns 字段<br/>创建 expert_sessions 表"]
CheckLegacy --> |不存在| Empty["返回空状态并注入默认值<br/>初始化 modelKits/userPreferences/failurePatterns 空集合<br/>创建 expert_sessions 表"]
...
## 依赖分析
- 类型依赖
  - service.ts 依赖 types.ts 中所有实体与输入输出类型，包括新增的 ModelKit、SubAgent、UserPreference、FailurePattern、ExpertSession、ExpertTaskNode、ExpertTask、AcceptanceTest、CodeResearchResult、AgentPool、TaskArtifact、ExpertError
...
```mermaid
graph LR
Types["types.ts<br/>新增 systemRole/UserPreference/FailurePattern"] --> Service["service.ts<br/>新增系统代理/偏好/失败模式/专家编排管理"]
...
**章节来源**
- [service.ts:43-2690](file://packages/core/src/service.ts#L43-L2690)
- [providers.ts:221-304](file://packages/core/src/providers.ts#L221-L304)
- [cli.ts:126-202](file://packages/core/src/cli.ts#L126-L202)
...
- 状态生命周期
  ...
  - 运行期：每次操作写回状态，事件与知识条目持续增长，ModelKit 配置按需更新，用户偏好和失败模式动态维护，专家会话按需创建和更新
...
- [service.ts:56-2690](file://packages/core/src/service.ts#L56-L2690)
- [seed-agents.ts:1-242](file://packages/core/src/seed-agents.ts#L1-L242)
- [habits.ts:1-149](file://packages/core/src/tools/habits.ts#L1-L149)
- [failure.ts:1-149](file://packages/core/src/tools/failure.ts#L1-L149)
- [knowledge.ts:1-175](file://packages/core/src/tools/knowledge.ts#L1-L175)
