...
## 更新摘要
**所做更改**
- 新增 ModelKit 配置模式和验证结构章节
- 更新核心组件部分，添加 ModelKit 和 SubAgent 相关实体
- 更新数据模型与关系图，包含新的 ModelKit 和 SubAgent 关系
- 新增 ModelKit 配置架构和验证规则章节
- 更新状态存储与缓存策略，包含 ModelKit 管理功能
- 新增 ModelKit API 端点和数据验证规则章节
- 更新扩展与定制指南，添加 ModelKit 和 SubAgent 扩展
...
graph TB
subgraph "核心类型"
T1["types.ts<br/>定义实体与枚举<br/>新增 ModelKit/SubAgent"]
end
subgraph "状态存储"
...
**图表来源**
- [types.ts:1-463](file://packages/core/src/types.ts#L1-L463)
- [store.ts:1-166](file://packages/core/src/store.ts#L1-L166)
- [service.ts:1-1702](file://packages/core/src/service.ts#L1-L1702)
...
- EngineConfig（引擎配置）
  - 字段：mode、cliId、cliModels、byokProviders、activeByokProviderId、modelKits、updatedAt
  - 模式：cli/byok；持久化模型选择与 BYOK 提供方
  - 新增：modelKits 字段，用于存储 ModelKit 集合
...
**章节来源**
- [types.ts:173-463](file://packages/core/src/types.ts#L173-L463)
- [service.ts:486-679](file://packages/core/src/service.ts#L486-L679)
...
**图表来源**
- [service.ts:56-1702](file://packages/core/src/service.ts#L56-L1702)
...
**章节来源**
- [types.ts:255-449](file://packages/core/src/types.ts#L255-L449)
- [service.ts:486-519](file://packages/core/src/service.ts#L486-L519)
- [index.ts:113-144](file://apps/server/src/index.ts#L113-L144)
- [index.ts:418-466](file://apps/server/src/index.ts#L418-L466)
...
## 结论
RepoHelm 的数据模型以 Quest 为核心，围绕 Workspace 与 Project 构建多项目任务工作流，结合 Git worktree 隔离与 Agent 后端编排，形成可审计、可扩展的状态体系。通过 SQLite 持久化与模型缓存提升性能，通过安全策略与审计日志保障执行安全。新增的 ModelKit 配置模式和 SubAgent 子代理系统进一步增强了模型管理和代理执行能力，为复杂任务场景提供了更好的支持。未来可在知识库检索、能力管理、产品化状态页、模型性能监控等方面进一步增强。
