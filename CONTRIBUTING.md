...
新 feature 或新能力请使用 feature request 模板。非平凡 feature 必须填写 `现状`、`目标`、`验收标准`、`社区方案调研` 和 `选型结论`。

修复类 issue 也必须写清：

- 现状：当前失败方式、影响范围和复现线索。
- 目标：修复后应该恢复或新增的行为。
- 验收标准：维护者如何用测试、命令、e2e 或人工步骤确认问题已解决。
...
5. PR 描述必须包含现状、目标、验收标准和测试结果。
6. 关联 issue；如果引入新能力或新依赖，摘要中说明复用选型结论。
