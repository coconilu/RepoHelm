...
- `pnpm test`：运行核心领域逻辑测试，覆盖 workspace bootstrap、SQLite 迁移、知识文件写入、Quest 创建、真实 worktree 创建、mock/CLI Agent 写入、diff 读取、worktree 清理、重试和交付 commit。
- `pnpm test:e2e`：运行 Playwright 浏览器测试，覆盖从 UI 创建 Quest、生成 Spec、运行 Quest、搜索 knowledge、展示 worktree/review/diff、交付、清理和 CLI backend 的主流程。
